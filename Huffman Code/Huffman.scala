...
object Huffman {
  ...
  {
    // Look up the first character encoded by bits.
    // Return that character along with the unused bits.
    def decodeChar(subTree: CodeTree, bits: List[Bit]): (Char, List[Bit]) = {
      subTree match {
        case Leaf(char, _) => (char,bits)  // We found the character
        case Fork(left, right, _, _) if (!bits.isEmpty) => decodeChar(if(bits.head==1) right else left,bits.tail) // Can ensure that bits is not empty.
        case _ => throw new Exception("Not enough bits") // Otherwise, (e.g., if bits is empty) throw an exception
      }
    }
    def decodeAcc(bits: List[Bit], acc: List[Char]): List[Char] = {
      if (bits.isEmpty) acc.reverse // We are done. Don't forget the accumulator is backwards.
      else {
        val charBits = decodeChar(tree, bits)  // Get the next character and the remaining bits
